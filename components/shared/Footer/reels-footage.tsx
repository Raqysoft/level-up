...
function ReelsFootage() {
  return (
    <div className="h-[140vh] grid grid-cols-10 grid-rows-1 gap-3">
      {/* Left */}
...
const FirstCol = ({ speakers, delay = 0 }: IColData) => {
  return (
    <div className="grid grid-rows-10 gap-3">
      ...
      >
        <ReelCard speaker={speakers[0]} className="w-full h-full" />
      </motion.div>
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        ...
        className="rounded-lg bg-zinc-600 row-span-3"
      >
        <ReelCard speaker={speakers[1]} className="w-full h-full" />
      </motion.div>
    </div>
  );
};
